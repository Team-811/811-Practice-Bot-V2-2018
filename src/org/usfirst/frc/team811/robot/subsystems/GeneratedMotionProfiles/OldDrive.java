...
public class OldDrive extends Subsystem implements Config {
    ...
    SetValueMotionProfile setValueLeft = SetValueMotionProfile.Disable;
    SetValueMotionProfile setValueRight = SetValueMotionProfile.Disable;
...
//		}
	...
	public void startFilling() {
		...
		if (statusLeft.hasUnderrun || statusRight.hasUnderrun) {
			/*
	...
	public void followTrajectory() {
		...
		leftMotor.getMotionProfileStatus(statusLeft);
		rightMotor.getMotionProfileStatus(statusRight);
		...
		if (leftMotor.getControlMode() != ControlMode.MotionProfile || rightMotor.getControlMode() != ControlMode.MotionProfile) {
			...
			switch (state) {
				...
				case 1: /*
					...
					if (statusLeft.btmBufferCnt > minBufferPoints && statusRight.btmBufferCnt > minBufferPoints) {
				...
				case 2: /* check the status of the MP */
					 ...
					 */
					if (statusLeft.isUnderrun == false || statusRight.isUnderrun == false) {
						loopTimeout = timeoutsMs;
					}
					/*
					...
					if (statusLeft.activePointValid && statusLeft.isLast) {
						 ...
						 */
						setValueLeft = SetValueMotionProfile.Hold;
						setValueRight = SetValueMotionProfile.Hold;
						state = 0;
						loopTimeout = -1;
						
					}
					if (statusRight.activePointValid && statusRight.isLast) {
						/*
						 * because we set the last point's isLast to true, we will
						 * get here when the MP is done
						 */
			...
			leftMotor.set(ControlMode.MotionProfile, setValueLeft.value);
			rightMotor.set(ControlMode.MotionProfile, setValueRight.value);
